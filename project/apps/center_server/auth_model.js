...
function _do_reg_phone_account(phone_num, pwd_md5,unick,ret_func){
	var usex = utils.random_int(0, 1); // 性别
	var uface = 0; // 系统只有一个默认的uface,要么就是自定义face;
	mysql_center.insert_phone_account_user(unick, uface, usex, phone_num,pwd_md5, function(status) {
		ret_func(status);
	});
}

function _do_account_reset_pwd(phone_num, pwd_md5,ret_func){
	mysql_center.reset_account_pwd( phone_num,pwd_md5, function(status) {
		ret_func(status);
	});
}
...
function _send_indentify_code(phone_num, opt, end_duration, ret_func) {
	...
	mysql_center.update_phone_indentify(code, phone_num, opt, end_duration, function(status) {
		if(status == Respones.OK) {
			// 发送短信
			phone_msg.send_identify_code(phone_num, code);
			// end 
		}
		ret_func(status);
	});
...
function get_upgrade_identify(uid, ukey, phone, opt, ret_func) {
	// 判断账号的合法性
...
function get_phone_reg_verify_code(phone_num,ret_func){
	mysql_center.check_phone_unuse(phone_num, function(status) {
		...
		// 手机绑定, 检查验证码
		_send_indentify_code(phone_num, 1, 60, ret_func);//操作数为1 代表要注册账号
		// end 
	});
}

function get_forget_pwd_verify_code(phone_num,ret_func){
	mysql_center.check_phone_is_reged(phone_num, function(status) {
		if(status != Respones.OK) {
			ret_func(Respones.PHONE_IS_NOT_REG);
			return;
		}
		// 手机绑定, 检查验证码
		_send_indentify_code(phone_num, 2, 60, ret_func);	//忘记密码 操作数为2
		// end 
	});
}
...
function _check_reg_phone_account_verify_code(phone_num, pwd_md5, verify_code,unick, ret_func){
	mysql_center.check_phone_code_valid(phone_num, verify_code, 1, function(status) {
		if (status != Respones.OK) {
			ret_func(Respones.PHONE_CODE_ERR);
			return;
		}
		...
		// 新建一个手机注册账号
		_do_reg_phone_account(phone_num, pwd_md5,unick,ret_func);
		// end 
	})
}

function _check_reset_pwd_verify_code(phone_num, pwd_md5, verify_code, ret_func){
	mysql_center.check_phone_code_valid(phone_num, verify_code, 2, function(status) {
		if (status != Respones.OK) {
			ret_func(Respones.PHONE_CODE_ERR);
			return;
		}
		...
		// 新建一个手机注册账号
		_do_account_reset_pwd(phone_num, pwd_md5,ret_func);
		// end 
	})
}
...
function guest_bind_phone_num(uid, phone_num, pwd_md5, phone_code, ret_func) {
	// 判断账号的合法性
	mysql_center.is_exist_guest(uid, function(status) {
		if(status != Respones.OK) {
			ret_func(Respones.INVALIDI_OPT);
...
function reg_phone_account(phone_num,pwd_md5,verify_code,unick,ret_func){
	mysql_center.check_phone_unuse(phone_num, function(status) {
		...
		// 手机注册, 检查验证码
		_check_reg_phone_account_verify_code(phone_num, pwd_md5, verify_code,unick, ret_func);
		// end 
	});
}
...
function reset_user_pwd(phone_num,pwd_md5,verify_code,ret_func){
	...
	_check_reset_pwd_verify_code(phone_num, pwd_md5, verify_code, ret_func);
	});
}
...
module.exports = {
	...
	get_upgrade_identify: get_upgrade_identify, // 游客升级，拉去手机验证码
	guest_bind_phone_num: guest_bind_phone_num, // 游客绑定手机号码;
	get_phone_reg_verify_code:get_phone_reg_verify_code,
	reg_phone_account:reg_phone_account,
	get_forget_pwd_verify_code:get_forget_pwd_verify_code,	//重置密码
