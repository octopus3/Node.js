...
function uname_login(session, utag, proto_type, body){
	if(!body || !body[0] || !body[1]) {
		session.send_cmd(Stype.Auth, Cmd.Auth.UNAME_LOGIN, Respones.INVALID_PARAMS, utag, proto_type);
		return;
	}
	// end 
...
function get_guest_upgrade_identify(session, uid, proto_type, body){
	if(!body || typeof(body[0]) == "undefined" || typeof(body[1]) == "undefined" || !is_phone_number(body[1]) || typeof(body[2]) == "undefined"){
		session.send_cmd(Stype.Auth, Cmd.Auth.GUEST_UPGRADE_IDENTIFY, Respones.INVALID_PARAMS, uid, proto_type);
		return;	
	}
	auth_model.get_upgrade_identify(uid,body[2],body[1],body[0],function(body){
...
function get_phone_reg_verify_code(session, utag, proto_type, body){
	if(!body || typeof(body[0]) == "undefined" || typeof(body[1]) == "undefined"){
...
function guest_bind_phone_num(session, uid, proto_type, body){ //body{0:phone_num,1:pwd,2:identify_code}
	if(!body || !body[0] || !body[1] || !body[2] ){
		session.send_cmd(Stype.Auth, Cmd.Auth.BIND_PHONE_NUM, Respones.INVALID_PARAMS, uid, proto_type);
	}
	auth_model.guest_bind_phone_num(uid,body[0],body[1],body[2],function(body){ //ret_func
		log.info(body);
...
function reg_phone_account(session, utag, proto_type, body){
    ...
    console.log("reg_phone_account was called");
	...
	on_recv_player_cmd: function(session, stype, ctype, body, utag, proto_type, raw_cmd) {
		...
		switch(ctype) {
			...
			case Cmd.Auth.GUEST_UPGRADE_IDENTIFY:
				get_guest_upgrade_identify(session, utag, proto_type, body);
			break;
			case Cmd.Auth.BIND_PHONE_NUM:
				guest_bind_phone_num(session, utag, proto_type, body);
			break;
			case Cmd.Auth.UNAME_LOGIN: 	//body{  0:ugame.uname,1:ugame.upwd}
