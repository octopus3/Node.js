...
// 创建一个连接池，每次要发送 sql语句给mysql server都通过这个连接池来获得连接对象;
// 连接对象的有效性就有这个池来负责管理;
...
function mysql_exec(sql, callback) {
	...
	conn_pool.getConnection(function(err, conn) {
		...
		// query向服务器发送sql语句的命令，有返回的话，就会调用回掉函数;
...
// end
// update一般不关心结果，但是他会给回一个update的结果的情况。
