...
function insert_phone_account_user(unick, uface, usex, phone_num,pwd_md5,callback){
	var sql = "insert into uinfo(`uname`, `upwd`,`unick`, `uface`, `usex`, `is_guest`)values(\"%s\",\"%s\", \"%s\", %d, %d, 0)";
	var sql_cmd = util.format(sql, phone_num,pwd_md5, unick, uface, usex);
	log.info(sql_cmd);
...
function reset_account_pwd(phone_num,pwd,callback){
	var sql = "update uinfo set  upwd = \"%s\", is_guest = 0 where uname = \"%s\"";
	var sql_cmd = util.format(sql, pwd, phone_num);
	log.info(sql_cmd);
